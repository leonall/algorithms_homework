...
def max_sliding_window_using_deque(arr, k):
    qi = deque()  # queue storing indexes of elements
    result = []
...
class TestSuite(unittest.TestCase):
    ...
    def test_max_sliding_window_using_queuq(self):
        for _ in range(10):
            arr = np.random.randint(0, 10, 10)
            k = np.random.randint(1, 9)
            self.assertEqual(max_sliding_window_using_deque(arr, k), max_sliding_window(arr, k))
